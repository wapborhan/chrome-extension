...
// Retrieve sites from chrome.storage
chrome.storage.local.get("sites", (result) => {
  if (result.sites) {
    site_data = result.sites;
    render();
  }
});
...
submit.addEventListener("click", () => {
  if (tagVAL.value && siteVAL.value) {
    ...
    if (check(tagVAL.value)) {
      ...
      errorINP.textContent = "Tag already exist, enter a unique tag.";
    }
  } else {
...
function check(ele) {
  for (let i = 0; i < site_data.length; i++) {
    if (site_data[i][0] == ele) return false;
  }
  return true;
}
...
function render() {
  ...
  for (let i = 0; i < site_data.length; i++) {
    ...
    tempid.push(
      task_obj.addEventListener("click", function () {
